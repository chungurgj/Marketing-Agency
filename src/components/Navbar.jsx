...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 z-50'>
        <div className={`fixed top-0 left-0 h-screen bg-white sm:hidden w-[65%] transform transition-all ease-in-out duration-300
        ${openSideMenu ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className='flex flex-col justify-between h-[50%] '>
            <div className='flex justify-between items-center py-[19px] px-4 bg-gray-700'>
            ...
            <ul className='flex flex-col justify-center items-center gap-2 '>
                <li className={`sidebar-element p-1 ${location.pathname === '/' ? 'active' : ''}`}><a href='/'>Home</a></li>
                <li className={`sidebar-element p-1 ${location.pathname.startsWith('/about') ? 'active' : ''}`}><a href='/about'>About Us</a></li>
                <li className={`sidebar-element p-1 ${location.pathname.startsWith('/services') ? 'active' : ''}`}><a href='/services'>Services</a></li>
            </ul>

            <ul className='flex justify-center gap-5'>
                <li><a href='/login' className='bg-accent p-1 px-3 rounded text-white'>Log in</a></li>
        ...
        <div className='container flex px-5 sm:px-1 justify-between items-center w-full '>
            ...
            <div className='sm:flex hidden gap-5 items-center'>
                <a href="/login" className='bg-accent p-1 px-3 rounded '>Log in</a>
